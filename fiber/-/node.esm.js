...
(function ($) {
    ...
    function $mol_fiber_func(calculate) {
        const wrapper = function $mol_fiber_func_wrapper(...args) {
    ...
    function $mol_fiber_method(obj, name, descr) {
        ...
        descr.value = function $mol_fiber_action_wrapper(...args) {
            const slave = $mol_fiber.current;
            let master = slave && slave.master;
            if (!master || master.constructor !== $mol_fiber) {
                master = new $mol_fiber;
        ...
        return function $mol_fiber_sync_wrapper(...args) {
            ...
            if (!master || master.constructor !== $mol_fiber) {
                master = $mol_fiber.make(fiber => {
                    fiber.cursor = -3;
                    fiber.error = request.call(this, ...args).then(res => fiber.push(res), err => fiber.fail(err));
                    const prefix = slave ? `${slave}/${slave.cursor / 2}:` : '/';
